...
const obtenerReporteData = async ({ tipo = "semanal", fecha = null }) => {
  ...
  const [
    ...
    AlertaCamioneta.countDocuments({ estado: { $in: ["ABIERTA", "EN_PROCESO"] }, ...fechaFiltro("fechaCreacion", inicio, fin) }),
    ...
    AlertaCamioneta.countDocuments({ prioridad: "CRITICA", estado: { $in: ["ABIERTA", "EN_PROCESO"] } }),
    ...
    AlertaCamioneta.countDocuments({ estado: { $in: ["ABIERTA", "EN_PROCESO"] }, ...fechaFiltro("fechaCreacion", inicioAnterior, finAnterior) }),
