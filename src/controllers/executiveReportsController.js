...
const obtenerReporteData = async ({ tipo = "semanal", fecha = null }) => {
  ...
  const [
    ...
    AlertaCamioneta.countDocuments({ estado: { $in: ["ABIERTA", "ASIGNADA", "EN_PROCESO"] }, ...fechaFiltro("fechaCreacion", inicio, fin) }),
    AlertaCamioneta.countDocuments({ estado: "RESUELTA", ...fechaFiltro("fechaResolucion", inicio, fin) }),
    AlertaCamioneta.countDocuments({ estado: "CERRADA", ...fechaFiltro("fechaCierre", inicio, fin) }),
    AlertaCamioneta.countDocuments({ prioridad: "CRITICA", estado: { $in: ["ABIERTA", "ASIGNADA", "EN_PROCESO"] } }),
    ...
    AlertaCamioneta.countDocuments({ estado: { $in: ["ABIERTA", "ASIGNADA", "EN_PROCESO"] }, ...fechaFiltro("fechaCreacion", inicioAnterior, finAnterior) }),
